...
class TestParser(BaseTests):
    ...
    def test_不应有歧义(self):
        ...
        @分析器母机.语法规则("语句 : 语句1 | 语句2")
        def 语句(片段):
            return 片段

        @分析器母机.语法规则("语句1 : 标识符 将 量 显示")
        def 语句1(片段):
            return 片段

        @分析器母机.语法规则("语句2 : 量 将 显示")
        def 语句2(片段):
            return 片段
        ...
        with py.test.raises(Exception) as 报错:
            分析器.分析(iter([
                词("标识符", "和"),
                词("将", "将"),
                词("显示", "显示"),
            ]))
        assert "此词需识别但位置不明：Token('显示', '显示')" in repr(报错.value)


    def test_改后无歧义报警(self):
        分析器母机 = 语法分析器母机(
            ["标识符", "将", "显示", "整数"]
        )
        @分析器母机.语法规则("语句 : 语句1 | 语句2")
        ...
        @分析器母机.语法规则("语句2 : 整数 将 显示")
        @分析器母机.语法规则("语句2 : 标识符 将 显示")
        def 语句2(片段):
            return 2
