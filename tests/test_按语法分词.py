...
class Test按语法分词(object):
    ...
    def test_建删表改删记录_生成SQL(self):
        ...
        分析器母机 = 语法分析器母机(['删除', '创建', '记录', '将', '获取', '改为', '为', '的', '整数', '不为空', '不重复', '自动递增', '小于', '大于', '主键', '文本', '表', '数', '标识符', '句号', '逗号', '顿号'])
        ...
        @分析器母机.语法规则("句 : 删表 | 建表 | 删记录 | 改记录 | 读记录")
        def 句(片段):
            return 片段[0]
        ...
        @分析器母机.语法规则("改记录 : 将 选取条件 的 标识符 记录 的 标识符 改为 数")
        def 改记录(片段):
            return f"UPDATE {片段[3].getstr()} SET {片段[-3].getstr()} = {片段[-1].getstr()} WHERE {片段[1]}"

        @分析器母机.语法规则("读记录 : 获取 选取条件 的 标识符 的 各相关值")
        def 读记录(片段):
            return f"SELECT {片段[5]} FROM {片段[3].getstr()} WHERE {片段[1]}"

        @分析器母机.语法规则("选取条件 : 标识符 小于 数 | 标识符 大于 数 | 标识符 为 数")
        def 选取条件(片段):
            if 片段[1].getstr() == "小于":
                运算符 = "<"
            elif 片段[1].getstr() == "大于":
                ...
                运算符 = "="
            return f"{片段[0].getstr()} {运算符} {片段[2].getstr()}"
        ...
        # 回退次数大是由于单句字数多导致标识符长度从19开始试。可能改进：作预遍历以避免从最大匹配开始尝试。
        assert 分析器.按语法分词(分词器.分词('将编号为1的读者记录的出生年改为2001。'), 261) == 'UPDATE 读者 SET 出生年 = 2001 WHERE 编号 = 1;'
        # 合并上两句，回退数为两者之和
        assert 分析器.按语法分词(分词器.分词('创建读者表，编号为自动递增的整数主键，邮箱为不重复不为空的文本，出生年为整数。将编号为1的读者记录的出生年改为2001。删除出生年小于2000的读者记录。删除读者表。'), 191 + 261 - 1) == 'CREATE TABLE 读者 ( 编号 INTEGER PRIMARY KEY AUTOINCREMENT, 邮箱 TEXT UNIQUE NOT NULL, 出生年 INTEGER ); UPDATE 读者 SET 出生年 = 2001 WHERE 编号 = 1; DELETE FROM 读者 WHERE 出生年 < 2000; DROP TABLE 读者;'

        assert 分析器.按语法分词(分词器.分词('获取出生年大于2000的读者的邮箱、昵称。'), 65) == 'SELECT 邮箱, 昵称 FROM 读者 WHERE 出生年 > 2000;'
