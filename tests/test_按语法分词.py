...
class Test按语法分词(object):
    ...
    def test_两句创建删除(self):
        ...
        分析器母机 = 语法分析器母机(['删除', '创建', '为', '的', '整数', '不为空', '文本', '表', '标识符', '句号', '逗号'])
        ...
        @分析器母机.语法规则("列声明 : 标识符 为 列类型 | 标识符 为 列特性 的 列类型")
        def 列声明(片段):
        ...
        assert 分析器.按语法分词(分词器.分词('创建读者表，编号为整数，昵称为不为空的文本，出生年为整数。删除读者表。'), 58) == '读者 各列为 编号 整数 昵称 不为空 文本 出生年 整数；删除 读者'
