...
class Test按语法分词(object):
    ...
    def test_建删表改删记录_生成SQL(self):
        ...
        分析器母机 = 语法分析器母机(['删除', '创建', '记录', '将', '改为', '为', '的', '整数', '不为空', '不重复', '自动递增', '小于', '主键', '文本', '表', '数', '标识符', '句号', '逗号'])
        ...
        @分析器母机.语法规则("选取条件 : 标识符 小于 数 | 标识符 为 数")
        def 选取条件(片段):
            运算符 = "<" if 片段[1].getstr() == "小于" else "="
