...
class Test按语法分词(object):
    ...
    def test_建删表改删记录_生成SQL(self):
        ...
        分析器母机 = 语法分析器母机(['删除', '创建', '记录', '将', '获取', '改为', '为', '的', '整数', '按', '排列', '倒序', '添加', '不为空', '不重复', '自动递增', '小于', '大于', '主键', '文本', '表', '数', '标识符', '句号', '逗号', '顿号'])
        ...
        assert 分析器.按语法分词(分词器.分词('添加读者记录，编号为1，出生年为2000。'), 25) == 'INSERT INTO 读者 (编号, 出生年) VALUES (1, 2000);'