...
class Test辩证(object):
    ...
    def test_复杂条件(self):
        ...
        分词器 = 分词母机.产出()
        分析器 = 分析器母机.产出()
        assert str(分析器.按语法分词(分词器.分词('甲有问题，或乙有问题，或丁有问题'), 50)) == 'Or(Or(甲, 乙), 丁)'
        条件 = '若甲有问题，而且丁没问题则乙有问题'
        assert str(分析器.按语法分词(分词器.分词(条件), 100)) == 'If(And(甲, Not(丁)), 乙, True)'
    ...
    def test_例程(self):
        ...
        条件2 = '若乙有问题则丙有问题。'
        条件3 = "甲有问题，而且乙有问题不可能。"
        assert 分析器.按语法分词(分词器.分词(条件0), 50) == '甲有问题乙没问题丙没问题丁没问题'
