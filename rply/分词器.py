...
class 词流(object):
    ...
    def next(自身):
        print("不符语法词法规则：" + str(自身.不符语法词法规则))
    ...
    # TODO：严重——应该按照状态记录，而非全局？“删除”可能应该在0位置，但因为后续位置的分词错误导致记录“‘删除’不可在0位置”
    def 标记不符语法词法规则(自身):
        #print(自身.上个位置)
        #print(自身.上个规则)
        # 不记录固定模式？
        ...
        # 回退读词位置到此词之前，以便尝试匹配下个词法规则。 TODO: 此处可能因为退回了上一行出问题。
        # 为避免与 回退() 冲突，加下面判断
        if 自身.上个位置 < 自身.位置:
            print("回退位置: " + str(自身.位置) + "->" + str(自身.上个位置))
            自身.位置 = 自身.上个位置
    ...
    # 返回：回退成功
    # 直接选取第一个更前的回退点。TODO：是否应回退到最近的？
    def 回退(自身):
        print("分词当前位置: " + str(自身.位置))
        已回退 = False
        for 回退位置 in 自身.回退点:
            if 回退位置 >= 自身.位置:
                continue
            ...
            if 回退位置 not in 自身.不符语法词法规则:
                自身.位置 = 回退位置
                print("未变长匹配过，回退到: " + str(回退位置))
                已回退 = True
                break
            for 规则 in 自身.不符语法词法规则[回退位置]:
                if 自身.变长模式已试长度[规则][回退位置] > 1:
                    自身.位置 = 回退位置
                    print("变长匹配尚未彻底尝试，回退到: " + str(回退位置))
                    已回退 = True
                    break
                if 已回退:
                    break
