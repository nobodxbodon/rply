...
class 词流(object):
    def __init__(自身, 分词器, 源码):
        ...
        自身.上个位置 = None
        自身.上个规则 = None
        # 同一位置可能多个词法规则无效，int->set TODO：此三个表需合并或清理
        自身.不符语法词法规则 = {}
        自身.变长模式已试长度 = {}
        自身.位置对应词法规则 = {}
    ...
    def next(自身):
        ...
        for 规则 in 自身.分词器.词规则:
            ...
            自身.上个位置 = 自身.位置
            if 匹配词:
                自身.位置对应词法规则[自身.位置] = 规则
                print(f"找到匹配词：{自身.源码[匹配词.起:匹配词.止]}, 路径历史：{自身.位置对应词法规则}")
                自身.上个规则 = 规则
                行号 = 自身._行号
                自身._列号 = 自身._更新位置(匹配词)
    ...
    def _匹配变长模式(自身, 规则):
        if 规则 in 自身.变长模式已试长度:
            if 自身.变长模式已试长度[规则][自身.位置] > 1:
                ...
                if 匹配词:
                    print(f"变长匹配于 {自身.位置} ：{自身.源码[匹配词.起:匹配词.止]}，规则：{规则}，长度：{再试长度}")
                    自身.变长模式已试长度[规则][自身.位置] = 再试长度
                    return 匹配词
                else:
                    print(f"未匹配变长：{规则}，指定长度：{再试长度}")
            else:
                print(f"变长模式已试完：{规则}")
        else:
    ...
    def 标记不符语法词法规则(自身):
        ...
        自身._记录问题规则(自身.上个位置, 自身.上个规则)

    def 标记分词失败规则(自身):
        相关规则 = 自身.位置对应词法规则[自身.位置]
        print(f"标记分词失败规则: {自身.位置} -> {相关规则}")
        自身._记录问题规则(自身.位置, 相关规则)

    def _记录问题规则(自身, 位置, 规则):
        if 位置 in 自身.不符语法词法规则:
            自身.不符语法词法规则[位置].add(规则)
        else:
            自身.不符语法词法规则[位置] = set([规则])
