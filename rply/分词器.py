...
class 词流(object):
    ...
    def next(自身):
        ...
        for 规则 in 自身.分词器.词规则:
            ...
            if 匹配词:
                if 自身._为变长匹配(规则):
                    匹配长度 = 匹配词.止 - 匹配词.起
                    print(f"变长匹配于 {自身.位置} ：{自身.源码[匹配词.起:匹配词.止]}，规则：{规则}，长度：{匹配长度}")
                    if 规则 not in 自身.变长模式已试长度:
                        自身.变长模式已试长度[规则] = {}
                    自身.变长模式已试长度[规则][自身.位置] = 匹配长度
                自身.当前路径各匹配词法[自身.位置] = 规则
                print(f"找到匹配词：{自身.源码[匹配词.起:匹配词.止]}, 路径历史：{自身.当前路径各匹配词法}")
    ...
    def 标记失败规则(自身):
        ...
        if 自身.位置 in 自身.不符语法词法规则:
            自身.不符语法词法规则[自身.位置].add(相关规则)
        else:
            自身.不符语法词法规则[自身.位置] = set([相关规则])
